...
pub struct ServerRequest<Stream: Read + Write> {
    stream: Option<Stream>,
}

impl<Stream: Read + Write + Debug> ServerRequest<Stream> {
    ...
    pub fn parse(stream: Stream) -> Result<http::Request<Body<Stream>>, HttpError> {
        ...
        loop {
            ...
            request = request.method(req.method.unwrap()).uri(req.path.unwrap()).version(version);
    ...
    pub fn respond<T: BufRead + Read + Write + HasLength + Debug + Clone>(
        stream: Stream, response: http::Response<T>) -> Result<Stream, HttpError> {
